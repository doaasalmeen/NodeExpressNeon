...
const securityMiddleware = async(req, res, next)=>{
    try{
        ...
        return next(); // اسمح بطلبات Postman
        }
    ...
    if(decision.isDenied()&& decision.isShield()){
    ...
    if(decision.isDenied()&& decision.isRateLimit()){
        logger.warn('Rate limit exceede', {ip: req.ip, userAgent: req.get('User-Agent'),path: req.path});
        return res.status(403).json({error: 'Forbidden', message: 'Too many requests'});
    }
    next();
    }catch(e){
        console.error('Arcjet middleware error:', e);
        res.status(500).json({error: 'Iternal server error',  message: 'Something went wrong with security middleware'})
    }
}
